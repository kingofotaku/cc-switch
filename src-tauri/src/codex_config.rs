...
fn uses_code_mode_only_gpt_catalog(slug: &str) -> bool {
...
fn use_stable_custom_openai_tool_surface(slug: &str, profile: CodexCatalogToolProfile) -> bool {
    profile == CodexCatalogToolProfile::ProxyChat && uses_code_mode_only_gpt_catalog(slug)
}
...
"#;
    ...
    #[test]
    fn proxy_chat_catalog_uses_stable_tool_surface_for_code_mode_only_gpt_custom_provider() {
        // GPT-5.6 and GPT-6 over a third-party/custom provider currently lose local
        ...
        for entry in catalog["models"].as_array().expect("catalog models") {
            ...
            assert_eq!(
                ...
                None,
                "custom-provider GPT must not enter the 3000 multi-agent tool path"
            );
            assert_eq!(
                entry.get("tool_mode"),
                None,
                "custom-provider GPT must not carry backend tool_mode"
            );
            assert_eq!(
                entry.get("shell_type").and_then(|v| v.as_str()),
                Some("shell_command")
            ...
            assert!(
                ...
                "GPT-specific reasoning levels must be preserved"
            );
        }
    }
