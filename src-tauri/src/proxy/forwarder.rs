...
mod tests {
    ...
    async fn codex_endpoint_failover_retries_current_three_times_then_persists_fallback() {
        ...
        let adapter = get_adapter(&AppType::Codex).expect("Codex adapter");
    ...
    #[test]
    fn prevention_skipped_for_vision_capable_deepseek_v4_aliases() {
        let fwd = forwarder_with_rectifier(RectifierConfig::default());
        let provider = provider_with_settings(json!({}));
        for model in ["deepseek-v4-pro", "deepseek-v4-flash"] {
            ...
            assert_eq!(replaced, 0, "V4 aliases route to vision-capable V4.1 Flash");
