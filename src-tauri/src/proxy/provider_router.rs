...
impl ProviderRouter {
    ...
    pub async fn select_providers(&self, app_type: &str) -> Result<Vec<Provider>, AppError> {
        ...
        // get_all_providers() also hydrates provider_endpoints into
        // ProviderMeta.custom_endpoints. The single-provider lookup cannot be
        // used here because Codex endpoint failover needs that data even when
        // provider-level failover is disabled.
        let current_provider = current_id
            .as_deref()
            .map(|id| {
                self.db
                    .get_all_providers(app_type)
                    .map(|providers| providers.get(id).cloned())
            })
            .transpose()?
            .flatten();
