...
pub fn resolve_codex_chat_reasoning_config(
    ...
    let mut config = match requested_model
        .as_deref()
        .and_then(|model| codex_model_catalog_reasoning_config(provider, model))
    {
        Some(config) => normalize_codex_chat_reasoning_config(config),
        None => {
