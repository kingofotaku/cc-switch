...
export interface ProviderMeta {
  ...
  // 请求失败时自动轮换同一 Codex 供应商的备用端点
  endpointFailoverEnabled?: boolean;
  // 是否为官方合作伙伴
  isPartner?: boolean;
...
export interface CodexCatalogModel {
  ...
  // Hidden per-model reasoning transport metadata. This is consumed by the
  // local Codex Chat conversion path and must survive provider edits.
  codexChatReasoning?: CodexChatReasoning;
  // Marks rows generated by the capability profile so the repair script may
  // refresh them while preserving operator-authored capability overrides.
  codexCapabilitySource?: "codex-model-profile" | string;
}
